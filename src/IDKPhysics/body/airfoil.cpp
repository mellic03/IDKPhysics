...
Airfoil::Airfoil( World &world, const glm::vec3 &pos, float span, float chord )
:   RigidBody(world, pos, new ShapeOBB(span, 1.0f, chord)),
    m_span          (span),
    m_chord         (chord),
...
Airfoil::_sampleLiftDragCoefficients( float aoa )
{
    // idk_printvalue(aoa);
...
glm::vec3
Airfoil::computeAeroForcesLinear()
{
    auto &conf = m_world.config;

    static auto wind = idk::randvec3(-0.2, +0.2);

    glm::vec3 airspeed = state.linear.vel - wind;
...
void
Airfoil::update( float alpha )
{
    RigidBody::update(alpha);
}


void
Airfoil::integrate( float dt )
{
